const Basket=({items,handleRemove,handleAdd,handleReduce})=>{
    let totalP=0;
    
    let disp=items.map((x)=>{
        let price=x.unitPrice*x.count;
        totalP+=price;
        return(
            
            <div className='tc dib br3 ma2 mw5 w-40' key={x.name}>
                <div className="bg-gray br3">
                    <h3>{x.name}</h3>
                    <img className='w-70 br3 h4' alt='prod'src={x.img}/>
                    <div>
                        <img onClick={()=>handleReduce(x.name)} alt="" src="data:image/png;base64,"/>              
                        <span className="pa3">{x.count}</span>
                        <img onClick={()=>handleAdd(x)} alt="" src="data:image/png;base64,"/>
                    </div>
                    <div>
                        <p className="b">
                            Price:KSh.{price}
                        </p>
                    
                    </div>
                </div>
               
                <button className="br3 bg-gray white pv2 f6" onClick={()=>handleRemove(x.name)}>REMOVE</button>
            </div>
          
        
         
        )
             
    })

    return(
        <div >
            {disp}<br/>
            <span className="f3 fw5"> TOTAL:<span className="green fw6">KSh{totalP}</span></span>
        </div>
    )

} 

export default Basket;